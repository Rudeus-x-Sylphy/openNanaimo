...
foreach($resourceBox in @($hpMaxBox,$mpMaxBox,$coinBox,$nanaPointBox,$cardKeyNormalBox,$cardKeyGoldBox,$cardKeyMysteryBox,$cardKeySpecialBox,$freeMagicKeyExpiryBox)){$resourceBox.add_ValueChanged({if($launchInfoBox){try{Update-LaunchPreview}catch{$launchInfoBox.Text=$_.Exception.Message}}})}
...
function Ensure-ClientCompatibility {
    ...
    $arguments=@($runtime.Prefix)+@($ClientCompatibilityTool,'--source-root',$Root,'--output-root',$ClientCompatibilityOverlay,'--furniture','--revival-display','--dungeon-state','--inventory-gift-display','--dungeon7','--overwrite','--apply')
...
if($SelfTestProfileIO){
    ...
    foreach($k in 'hp_max','mp_max','attack','defense','coin','nana_point','card_key_normal','card_key_gold','card_key_mystery','card_key_special','free_magic_key_expiry','quickbar_expiry'){if([string]$roundIni[$k]-ne[string]$expect[$k]){throw "INI roundtrip $k"};if([string]$roundJson.resources.$k-ne[string]$expect[$k]){throw "JSON roundtrip $k"}}
