...
“一键进入 Nanaimo”会应用界面中的最新配置。等级只用于新角色的初始种子；已有档案保留游戏中累计的等级和经验。HP/MP、攻击、防御、货币、外观、装备宠物、技能和库存管理数据会在启动前同步。地图、页面和坐标仍写入人物档案，但新的客户端进程不会在首个 C367 直接套用任意上次页面。

## 登录出生点与位置持久化

- 新角色首次进入仍走游戏原始的角色创建与新手引导流程，不预置到村庄页面。
- 完成引导后，角色的地图、村庄页面和坐标继续写入人物档案，供会话内保存、诊断和后续兼容研究使用。
- 原客户端在每个新进程的第一次 C355 后会进入一次 `FirstVillageFlag` 角色构造分支；该分支忽略 C368 的 X/Y，改读当前页面内建的首次进入点。把任意存档页面提前写入 C355 会在缺少该入口的页面得到 `CVillageChar(-1,-1)`。
- 当前发布因此让已完成引导的角色先经已验证的 `map/page=0/0` 启动页进入，并且不在首个 C367 上覆盖为存档页面。跨进程直接恢复任意页面暂时停用，直到逐页入口或客户端兼容补丁得到验证。
- 安全启动角色建立后，正常 C365/C367/CB21 移动仍更新并保存 HP/MP、地图、页面和合法坐标。
- C365/C367/CB21 中的 `FFFF/FFFF` 表示场景切换／活动哨兵。适配器保持请求驱动路由，不把该哨兵或旧的 `1023,1023` 污染值写回人物档案；需要坐标回退时使用 `(400,96)`。
