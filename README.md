...
“一键进入 Nanaimo”会应用界面中的最新配置。对已有档案，HP/MP、货币、外观、装备宠物、技能和库存管理数据会在启动前同步；人物等级、经验和上次下线位置保持不变。
...
本仓库不提供客户端。使用与记录基线一致的客户端根目录，并按 [完整 adapter 与客户端补丁指引](docs/完整适配器与客户端补丁指引.md) 修复以下两个已确认问题：

1. **家具选择崩溃 bug**：校验 `game.exe` 为记录基线后，在 `VA 0x0041235F` 将跳转目标由 `0x005CE700` 改为原生 Index getter `0x005CEFE0`，即把 `E9 9C C3 1B 00` 替换为 `E9 7C CC 1B 00`。
