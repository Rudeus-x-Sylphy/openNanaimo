...
2026年9月26日当前发布候选统一了背包实例、消费、扩充、卸装与战斗资源生命周期：购物券和普通道具按领域路由；快捷栏、丢弃、材料、通用使用和扩充券共享稳定的会话实例身份；城镇食物按实际 HP/MP 增量事务提交；外观零槽作为真实卸装状态持久化；GUI/DB 保存基础最大值，选中宠物宝石形成 effective 最大值，absolute current 跨库存刷新、结算、连续关卡、回城和重启保持一致。地宫手动结算、超级 Boss 评分、分体目标账本与多阶段终结继续使用统一状态周期。源码、runtime、清单、确定性原生产物和发布包验证均以本候选为准。
